...
`discoverAndLoadExtensions` (`loader.ts:678`) is a self-contained scanner:
...
(`package-manager.ts:901`), which delegates the filesystem scan to
`addAutoDiscoveredResources` (`package-manager.ts:2309`, called at `:950`).
That resolver has its own
precedence — project `.pi/extensions/` first (only when
`settingsManager.isProjectTrusted()`, `package-manager.ts:2374`), then user
`~/.pi/agent/extensions/` (`package-manager.ts:2427`), both merged with
...
coding-agent package itself) into `VIRTUAL_MODULES` (`loader.ts:48-72`), and
...
mode uses a parallel `getAliases()` map instead (`loader.ts:84-140`); the two
lists must stay in sync.

### `tool_call` handlers can short-circuit by returning `{ block: true }`

`ExtensionRunner.emitToolCall` (`runner.ts:927`) iterates every loaded
...
(`runner.ts:938-943`.) Later extensions do not run after a block.
...
`pi.registerTool(tool: ToolDefinition<...>)` (`types.ts:1238`) takes the exact
